...
func Run(main func() error) (err error) {
	...
	case err = <-finish:
		if err != nil {
			plog.Warnf(nil, `lib.application, stop on finish, error:%s`, err.Error())
		}
	case <-sig:
		plog.Warn(nil, `lib.application, stop on signal`)
	}
...
func safeExit() {
	...
	for _, handler := range exitHandlers {
		...
		go func(handler func()) {
			defer func() {
				if r := recover(); r != nil {
					plog.Warnf(nil, `lib.application, stop on safe exit, error:%s`, r)
				}
				wg.Done()
			}()
			handler()
